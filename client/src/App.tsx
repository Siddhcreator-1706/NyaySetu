...
function App() {
  ...
  return (
    <div className="app-container h-[100dvh] flex flex-col overflow-hidden" style={{ background: 'var(--bg-root)' }}>
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 py-6 font-[Inter,sans-serif] flex-1 flex flex-col min-h-0 overflow-hidden">

        {/* ═══════════════ HEADER ═══════════════ */}
        <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fade-in shrink-0">
          <div className="flex items-center gap-4">
            {/* Logo mark */}
            <div
              className="w-11 h-11 shrink-0 rounded-xl flex items-center justify-center text-xl shadow-lg leading-none"
              style={{
        ...
        {/* ═══════════════ TAB BAR ═══════════════ */}
        <div
          className="flex items-center gap-1 p-1 rounded-xl mb-6 w-fit shrink-0"
          style={{
        ...
        {/* ═══════════════ SQL EDITOR TAB ═══════════════ */}
        <div className={`animate-fade-in-up flex-1 flex flex-col min-h-0 overflow-hidden ${activeTab === 'sql' ? 'flex' : 'hidden'}`}>
          {/* Editor Card */}
          <div
            className="rounded-2xl p-5 mb-6 shrink-0"
            style={{
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 mb-3">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full sm:w-auto font-semibold py-2 px-5 rounded-lg transition-all disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer border-none text-white text-sm"
                  style={{
                    background: 'var(--gradient-primary)',
          ...
          {/* Results Card */}
          <div
            className="rounded-2xl p-4 sm:p-6 flex-1 flex flex-col min-h-0 overflow-hidden"
            style={{
              background: 'var(--bg-card)',
              border: '1px solid var(--border-default)',
              boxShadow: 'var(--glow-card)',
          ...
          >
            {/* Results header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 mb-4 shrink-0">
            ...
            {error && !loading && (
              <div
                className="p-4 rounded-xl text-sm whitespace-pre-wrap my-2 animate-fade-in overflow-x-auto shrink-0"
                style={{
            ...
            {!loading && !error && rows.length > 0 && (
              <div
                className="overflow-x-auto overflow-y-auto flex-1 rounded-xl animate-fade-in-up"
                  ...
                  <thead>
                    <tr style={{ background: 'rgba(99,102,241,0.06)' }}>
                      {columns.map((col) => (
                        <th
                          key={col}
                          className="p-3 font-semibold text-xs tracking-wide whitespace-nowrap sticky top-0"
                          style={{
                            color: '#a5b4fc',
                    ...
                    {rows.map((row, rIdx) => (
                      ...
                      >
                        {columns.map((col) => (
                          <td
                            key={col}
                            className="p-3 text-sm whitespace-nowrap max-w-[250px] truncate"
                            style={{ color: '#cbd5e1' }}
            ...
            {/* Empty state */}
            {!loading && !error && rows.length === 0 && (
              <div className="flex-1 flex flex-col justify-center items-center py-16 gap-3 text-center px-4">
                <div
        ...
        {/* ═══════════════ AI ASSISTANT TAB ═══════════════ */}
        <div
          className={`rounded-2xl p-4 sm:p-6 animate-fade-in-up flex-1 flex flex-col min-h-0 overflow-hidden ${activeTab === 'ai' ? 'flex' : 'hidden'}`}
          style={{
            background: 'var(--bg-card)',
            border: '1px solid var(--border-default)',
            boxShadow: 'var(--glow-card)',
